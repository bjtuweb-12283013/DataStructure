...
switch ($action)
{
	...
	case "del":
	$db->query("delete from ve123_tg_user where user_id='".intval($_GET["user_id"])."'");
	break;
}	
?>
<table width="100%" border="0" cellpadding="3" cellspacing="1" class="tablebg">
  ...
  <?php
  $result=$db->query("select * from ve123_tg_user");
...
global $db,$p_cid;
  if ($do_action=="modify")
  {
	  ...
	  $sql="select * from ve123_tg_user where user_id='$user_id'";
	  $rs=$db->get_one($sql);
...
global $db;
   ...
   if ($do_action=="modify")
   {
	 $array=array('user_name'=>$user_name,'email'=>$email);
	 $db->update("ve123_tg_user",$array,"user_id='$user_id'");
	 jsalert("修改成功");
   }
   else
   {     
             $array=array('user_name'=>$user_name,'email'=>$email);
	         $db->insert("ve123_tg_user",$array);  
			 jsalert("提交成功");
   }
}
?>
